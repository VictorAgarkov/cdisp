...
#if defined CDISP_MONOBMP_WIDTH && defined CDISP_MONOBMP_HEIGHT
	char cdisp_2_mono = 0;
	unsigned char cdisp_monobmp_buff[CDISP_MONOBMP_WIDTH * CDISP_MONOBMP_HEIGHT_L];
	#define cdisp_setpixel(a,b,c)        if(cdisp_2_mono) cdisp_monobmp_set_pixel(a,b,c);       else LCD_setpixel(a,b,c);
	#define cdisp_put_vconst_16(a,b,c,d) if(cdisp_2_mono) cdisp_monobmp_put_vconst_16(a,b,c,d); else LCD_put_vconst_16(a,b,c,d);
#else
	#define cdisp_setpixel       LCD_setpixel
	#define cdisp_put_vconst_16  LCD_put_vconst_16
#endif
...
void cdisp_rect_fill_r(union rect *R)
{
	...
	for(i = R->x1; i <= R->x2; i++)
	{
		cdisp_put_vconst_16(i, R->y1, h, RGB_24_to_16(current_color.l));
	}
}
//------------------------------------------------------------------------------
...
void cdisp_rect_holl_r(union rect *R)
{
	...
	cdisp_put_vconst_16(R->x1, R->y1, h, b16);

	for(i = R->x1; i <= R->x2; i++)
	{
		cdisp_setpixel(i, R->y1, b16);
		cdisp_setpixel(i, R->y2, b16);
	}

	cdisp_put_vconst_16(R->x2, R->y1, h, b16);
}
//------------------------------------------------------------------------------
void  cdisp_cyrcle_fill(short x, short y, unsigned short radius)
{
	...
	do
	{
		...
		{
			h = a * 2 + 1;
			cdisp_put_vconst_16(x+b, y-a, h, b16);
			cdisp_put_vconst_16(x-b, y-a, h, b16);
		}

		cdisp_put_vconst_16(x+a, y-b, b * 2 + 1, b16);
		if(a)
		{
			cdisp_put_vconst_16(x-a, y-b, b * 2 + 1, b16);
		}
...
//------------------------------------------------------------------------------
#if defined CDISP_MONOBMP_WIDTH && defined CDISP_MONOBMP_HEIGHT
	//------------------------------------------------------------------------------
	void cdisp_monobmp_set_pixel(short x, short y, unsigned short c)
	{
		if(x < 0 || x >= CDISP_MONOBMP_WIDTH || y < 0 || y >= CDISP_MONOBMP_HEIGHT) return;
		unsigned short offs = x * CDISP_MONOBMP_HEIGHT_L + (y >> 3);
		unsigned char bitm = 1 << (y & 7);
		if((c & 1) != 0)	cdisp_monobmp_buff[offs] |=  bitm;
		else              cdisp_monobmp_buff[offs] &= ~bitm;
	}
	//------------------------------------------------------------------------------
	void cdisp_monobmp_put_vconst_16(short x, short y, int num, unsigned short c)
	{

		short yy, y2 = min(y + num - 1, CDISP_MONOBMP_HEIGHT);
		short offs;
		if(x < 0 || x >= CDISP_MONOBMP_WIDTH || y < 0 || y >= CDISP_MONOBMP_HEIGHT) return;

		offs = x * CDISP_MONOBMP_HEIGHT_L + (y >> 3);

		unsigned char byte_val = ((c & 1) == 0) ? 0 : 0xff;
		...
		do
		{
			...
			cdisp_monobmp_buff[offs] = (cdisp_monobmp_buff[offs] & ~mask) | (byte_val & mask);
	...
	//------------------------------------------------------------------------------
	void cdisp_monobmp_flush(short x1, short y1, short x2, short y2)
	{
		// x1, x2, y1, y2 - destination rectangle on screen
		// x1, y1 equal to x0, y0 of mono bitmap
		short x, y, y0;
		unsigned short col0, col1, ccol;
		short num;
		...
		num = y2 - y1 + 1;
		col0 = RGB_24_to_16(current_color.l);
		col1 = RGB_24_to_16(current_font_color.l);

		for(x = max(x1, 0); x <= x2; x++)
		{
			short xm = x - x1;
			short offsx = xm * CDISP_MONOBMP_HEIGHT_L;
			for(y = y0; y <= y2; y++)
			{
				short offsy;
				...
				if(xm < CDISP_MONOBMP_WIDTH && ym < CDISP_MONOBMP_HEIGHT)
				{
					offsy = ym >> 3;
					mask = 1 << (ym & 7);
					b = cdisp_monobmp_buff[offsx + offsy];
					ccol = ((b & mask) == 0) ? col0 : col1;
				}
				else
				{
					ccol = col0;
			...
			LCD_put_vline_16(x, y0, num, line16_buff + y0);
		}
	}
	//------------------------------------------------------------------------------

#endif // CDISP_MONOBMP_WIDTH
//------------------------------------------------------------------------------
